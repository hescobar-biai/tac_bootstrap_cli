...
class TestUpgradeService:
    ...
    def test_get_current_version_no_version_field(self, tmp_path: Path) -> None:
        """get_current_version should return '0.1.0' for pre-0.9.4 projects without version field.
        """
        # Create config without version field (pre-0.9.4 projects)
        config_file = tmp_path / "config.yml"
        config_data = {
    ...
    def test_needs_upgrade_true(self, mock_project: Path) -> None:
        ...
        with patch("tac_bootstrap.application.upgrade_service.__version__", "0.9.4"):
