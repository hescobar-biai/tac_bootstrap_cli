...
class TestScaffoldServiceADWCompleteness:
    ...
    def test_config_yml_template_renders_version_fields(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert (
                f"Generated by TAC Bootstrap v{__version__}" in content
            ), f"Header should contain TAC version {__version__}"


# ============================================================================
# TEST SHARED INFRASTRUCTURE GENERATION
# ============================================================================


class TestScaffoldServiceSharedInfrastructure:
    """Tests for conditional shared infrastructure generation (DDD/Clean/Hexagonal + FastAPI)."""

    def test_scaffold_ddd_fastapi_includes_shared_infrastructure(self, service: ScaffoldService):
        """build_plan with DDD + FastAPI should include shared infrastructure."""
        config = TACConfig(
            project=ProjectSpec(
                name="test-ddd",
                language=Language.PYTHON,
            ...
            commands=CommandsSpec(start="uv run python -m app", test="uv run pytest"),
            claude=ClaudeConfig(settings=ClaudeSettings(project_name="test-ddd")),
        )
        ...
        assert "src/shared" in dir_paths
        assert "src/shared/domain" in dir_paths
        assert "src/shared/application" in dir_paths
        assert "src/shared/infrastructure" in dir_paths
        assert "src/shared/api" in dir_paths

        # Assert __init__.py files
        assert "src/shared/__init__.py" in file_paths
        assert "src/shared/domain/__init__.py" in file_paths
        assert "src/shared/application/__init__.py" in file_paths
        assert "src/shared/infrastructure/__init__.py" in file_paths
        assert "src/shared/api/__init__.py" in file_paths

        # Assert domain files
        assert "src/shared/domain/base_entity.py" in file_paths
        assert "src/shared/domain/base_schema.py" in file_paths

        # Assert application files
        assert "src/shared/application/base_service.py" in file_paths

        # Assert infrastructure files
        assert "src/shared/infrastructure/base_repository.py" in file_paths
        assert "src/shared/infrastructure/base_repository_async.py" in file_paths
        assert "src/shared/infrastructure/database.py" in file_paths
        assert "src/shared/infrastructure/exceptions.py" in file_paths
        assert "src/shared/infrastructure/responses.py" in file_paths
        assert "src/shared/infrastructure/dependencies.py" in file_paths

        # Assert API files
        assert "src/shared/api/health.py" in file_paths

        # All should use CREATE action
        shared_files = [f for f in plan.files if f.path.startswith("src/shared/")]
        for file_op in shared_files:
            assert file_op.action == FileAction.CREATE

    def test_scaffold_simple_fastapi_excludes_shared_infrastructure(self, service: ScaffoldService):
        """build_plan with SIMPLE + FastAPI should NOT include shared infrastructure."""
        config = TACConfig(
            project=ProjectSpec(
                name="test-simple",
                language=Language.PYTHON,
            ...
            commands=CommandsSpec(start="uv run python -m app", test="uv run pytest"),
            claude=ClaudeConfig(settings=ClaudeSettings(project_name="test-simple")),
        )
        ...
        # Assert no src/shared/ operations
        assert not any("src/shared" in path for path in dir_paths)
        assert not any("src/shared" in path for path in file_paths)

    def test_scaffold_ddd_django_excludes_shared_infrastructure(self, service: ScaffoldService):
        """build_plan with DDD + Django should NOT include shared infrastructure (FastAPI-only)."""
        config = TACConfig(
            project=ProjectSpec(
                name="test-ddd-django",
                language=Language.PYTHON,
                package_manager=PackageManager.UV,
                architecture=Architecture.DDD,
                framework=Framework.DJANGO,
            ),
            commands=CommandsSpec(start="python manage.py runserver", test="pytest"),
            claude=ClaudeConfig(settings=ClaudeSettings(project_name="test-ddd-django")),
        )
        ...
        # Assert no src/shared/ operations (framework must be FastAPI)
        assert not any("src/shared" in path for path in dir_paths)
        assert not any("src/shared" in path for path in file_paths)

    def test_scaffold_clean_fastapi_includes_shared_infrastructure(self, service: ScaffoldService):
        """build_plan with CLEAN + FastAPI should include shared infrastructure."""
        config = TACConfig(
            project=ProjectSpec(
                name="test-clean",
                language=Language.PYTHON,
            ...
            commands=CommandsSpec(start="uv run python -m app", test="uv run pytest"),
            claude=ClaudeConfig(settings=ClaudeSettings(project_name="test-clean")),
        )

        plan = service.build_plan(config)
        file_paths = [f.path for f in plan.files]

        # Assert all 10 base class files are included
        assert "src/shared/domain/base_entity.py" in file_paths
        assert "src/shared/domain/base_schema.py" in file_paths
        assert "src/shared/application/base_service.py" in file_paths
        assert "src/shared/infrastructure/base_repository.py" in file_paths
    ...
    def test_scaffold_hexagonal_fastapi_includes_shared_infrastructure(
        self, service: ScaffoldService
    ):
        """build_plan with HEXAGONAL + FastAPI should include shared infrastructure."""
        config = TACConfig(
            project=ProjectSpec(
                name="test-hexagonal",
                language=Language.PYTHON,
            ...
            commands=CommandsSpec(start="uv run python -m app", test="uv run pytest"),
            claude=ClaudeConfig(settings=ClaudeSettings(project_name="test-hexagonal")),
        )
    ...
    def test_apply_plan_creates_shared_infrastructure_files(self, service: ScaffoldService):
        """apply_plan should create all shared infrastructure files on disk."""
        config = TACConfig(
            project=ProjectSpec(
                name="test-apply-shared",
                language=Language.PYTHON,
                package_manager=PackageManager.UV,
                architecture=Architecture.DDD,
                framework=Framework.FASTAPI,
            ...
            commands=CommandsSpec(start="uv run python -m app", test="uv run pytest"),
            claude=ClaudeConfig(settings=ClaudeSettings(project_name="test-apply-shared")),
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert (tmp_path / "src" / "shared" / "domain").is_dir()
            assert (tmp_path / "src" / "shared" / "application").is_dir()
            assert (tmp_path / "src" / "shared" / "infrastructure").is_dir()
            assert (tmp_path / "src" / "shared" / "api").is_dir()

            # Verify __init__.py files exist
            assert (tmp_path / "src" / "shared" / "__init__.py").is_file()
            assert (tmp_path / "src" / "shared" / "domain" / "__init__.py").is_file()
            assert (tmp_path / "src" / "shared" / "application" / "__init__.py").is_file()
            assert (tmp_path / "src" / "shared" / "infrastructure" / "__init__.py").is_file()
            assert (tmp_path / "src" / "shared" / "api" / "__init__.py").is_file()

            # Verify base class files exist
            assert (tmp_path / "src" / "shared" / "domain" / "base_entity.py").is_file()
            assert (tmp_path / "src" / "shared" / "domain" / "base_schema.py").is_file()
            assert (tmp_path / "src" / "shared" / "application" / "base_service.py").is_file()
            base_repo = tmp_path / "src" / "shared" / "infrastructure" / "base_repository.py"
            assert base_repo.is_file()
            base_repo_async = (
                tmp_path / "src" / "shared" / "infrastructure" / "base_repository_async.py"
            )
            assert base_repo_async.is_file()
            assert (tmp_path / "src" / "shared" / "infrastructure" / "database.py").is_file()
            assert (tmp_path / "src" / "shared" / "infrastructure" / "exceptions.py").is_file()
            assert (tmp_path / "src" / "shared" / "infrastructure" / "responses.py").is_file()
            assert (tmp_path / "src" / "shared" / "infrastructure" / "dependencies.py").is_file()
            assert (tmp_path / "src" / "shared" / "api" / "health.py").is_file()

    def test_shared_infrastructure_renders_with_valid_python(self, service: ScaffoldService):
        """Shared infrastructure files should render with valid Python content."""
        config = TACConfig(
            project=ProjectSpec(
                name="test-render",
                language=Language.PYTHON,
            ...
            commands=CommandsSpec(start="uv run python -m app", test="uv run pytest"),
            claude=ClaudeConfig(settings=ClaudeSettings(project_name="test-render")),
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Check base_entity.py has expected content
            base_entity = tmp_path / "src" / "shared" / "domain" / "base_entity.py"
            content = base_entity.read_text()
            assert "from pydantic import" in content
            assert "class" in content
            assert "Entity" in content

            # Check base_repository.py has expected content
            base_repo = tmp_path / "src" / "shared" / "infrastructure" / "base_repository.py"
            content = base_repo.read_text()
            assert "from sqlalchemy" in content or "SQLAlchemy" in content
            assert "class" in content
            assert "Repository" in content or "def" in content

            # Check health.py has expected content
            health = tmp_path / "src" / "shared" / "api" / "health.py"
