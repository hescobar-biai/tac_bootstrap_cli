...
Example:
  <source-dir>/shared/domain  -> docs/<source-dir>/shared/domain.md
  <source-dir>/shared        -> docs/<source-dir>/shared.md
  <source-dir>              -> docs/<source-dir>.md

Key behavior
------------
- Bottom-up: deeper folders are generated first, then parents (parents can summarize children)
- Merge strategy:
  - If a docs file already exists, it is "complemented" conservatively:
...
- Uses an OpenAI-compatible model for summarization + structured doc generation
- Uses IDK vocabulary idea:
    * global/canonical IDK comes from any existing docs frontmatter in docs/**
    * local IDK is generated for each folder doc
    * IDK is keyword-only (no sentences)

Requires pydantic. Frontmatter parsing is minimal but reliable for typical YAML (no PyYAML).

Usage
-----
  python docs_fractal_generator.py \
    --repo . \
...
For local OpenAI-compatible servers (Ollama/LM Studio/vLLM):
  --base-url http://localhost:11434/v1 --api-key ollama --model qwen2.5:14b-instruct (example)
"""
...
def parse_scalar(yaml_text: str, key: str) -> str | None:
    m = re.search(rf"(?m)^\s*{re.escape(key)}\s*:\s*(.+?)\s*$", yaml_text)
    if not m:
        return None
    v = m.group(1).strip()
...
def load_canonical_idk_from_docs(docs_root: Path) -> dict[str, list[str]]:
    """
    domain -> merged unique idk across docs frontmatters in docs_root.
    domain is based on first segment of the docs-relative path.
    """
    ...
    for p in docs_root.rglob("*.md"):
        ...
        if not fm:
            continue
        idk = parse_list_block(fm, "idk")
        if not idk:
            continue
        rel = repo_rel(docs_root.parent, p)  # docs/... relative to repo
        docs_rel = rel.split("docs/", 1)[-1]
        dom = detect_domain_from_relpath(docs_rel)
        if dom not in out:
            out[dom] = []
            seen[dom] = set()
        for t in idk:
...
def extract_jsdoc_block(lines: list[str], symbol_line_idx: int, lookback: int = 40) -> str | None:
...
def extract_ts_doc_items(ts_path: Path) -> list[TsDocItem]:
    ...
    for i, ln in enumerate(lines):
        for pat, kind in TS_EXPORT_PATTERNS:
            ...
            if m:
                name = m.group(1)
                block = extract_jsdoc_block([line.rstrip("\n") for line in lines], i)
                if block:
...
def prompt_folder_doc(
    ...
    """
    system = (
        "You are a senior software documentation writer for a fractal docs system.\n"
...
def complement_doc(existing_md: str, generated_md: str) -> str:
    """
    Conservative merge:
    - Replace/normalize frontmatter with generated frontmatter
...
def extract_md_section(md_body: str, header: str) -> str | None:
    """
...
def run() -> int:
    ...
    ap.add_argument("--include-root", default="src", help="Folder to document (e.g., src, apps, services)")
    ...
    ap.add_argument("--model", default="cogito:8b")
    ...
    # Canonical/global IDK from existing docs frontmatter
    canonical_idk_by_domain = load_canonical_idk_from_docs(docs_root)
    ...
    for folder in folders:
        ...
        for f in folder.iterdir():
            ...
            elif f.suffix in {".ts", ".tsx"} and not f.name.endswith(".d.ts"):
        ...
        # Example: <source-dir>/domain/... => domain
        dom = "unknown"
        parts = folder_rel.split("/")
