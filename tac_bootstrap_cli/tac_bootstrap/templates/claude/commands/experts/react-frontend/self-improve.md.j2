...
1. **Check Git Diff (Conditional)**
   - If CHECK_GIT_DIFF is "true", run `git diff` to identify recent changes to frontend files
   - If CHECK_GIT_DIFF is "false", skip this step

2. **Read Current Expertise**
   - Read the entire EXPERTISE_FILE
   - Identify key sections: overview, component_architecture, data_fetching, state_management, styling
   - Note any areas that seem outdated or incomplete

3. **Validate Against Codebase**
   - Read documented key implementation files
   - Compare documented expertise against actual React components, hooks, and configurations
...
6. **Enforce Line Limit**
   - Run: `wc -l .claude/commands/experts/react-frontend/expertise.yaml`
   - If line count > MAX_LINES: trim least critical sections

7. **Validation Check**
   - Validate YAML syntax: `python3 -c "import yaml; yaml.safe_load(open('.claude/commands/experts/react-frontend/expertise.yaml'))"`
