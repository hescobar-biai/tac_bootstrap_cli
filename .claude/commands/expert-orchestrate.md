...
- 🔍 Planning: Create implementation plan with expert
- 🔨 Building: Implement the plan
- ✨ Improving: Validate and improve with expert
```
...
```
🔍 Step 1/3: Planning with tac_bootstrap {EXPERT_DOMAIN} expert...
```
...
- If cannot extract plan path: Display error below, ABORT workflow
  ```
  ❌ Could not extract plan file path from planning step.
...
**Display progress marker:**
```
🔨 Step 2/3: Building implementation from plan...
```
...
If file doesn't exist, display error and ABORT:
  ❌ Plan file not found: {PLAN_FILE_PATH}
```
...
**Display progress marker:**
```
✨ Step 3/3: Validating and improving with expert...
```
...
```
✅ Expert orchestration completed successfully!

Workflow: Plan → Build → Improve
Duration: 3 phases
...
- Self-improve fails → WARN (implementation exists, validation failed)
...
1. ✅ Domain and task description validated
2. ✅ Planning phase completes and produces plan file
3. ✅ Plan file path extracted successfully
4. ✅ Build phase completes implementation
5. ✅ Self-improve phase completes (or warns gracefully)
6. ✅ Synthesis report generated with all three phases
7. ✅ Clear progress markers shown throughout
8. ✅ Proper error handling with abort-on-failure
...
- Planning: `/experts:{domain}:plan [task]` → outputs plan file path
- Self-improve: `/experts:{domain}:self-improve [check_git_diff]` → validates expertise

**Build Command:**
- `/build [plan_path]` → implements plan from file path
